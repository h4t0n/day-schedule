...
function daySchedule(options, callback) {
    ...
    let ret = [];
    for (let i in times) {
        schedule.scheduleJob(times[i], function (theTime) {
            roundCallback[cbIndex++ % roundCallback.length](theTime);
        }.bind(null, times[i]));
        ret[i] = {
            date: times[i],
            functionName: roundCallback[cbIndex++ % roundCallback.length].name
        }
    }
